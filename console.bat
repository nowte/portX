...
if "%UPDATECMD%"=="1" (
    call python "%ENGINEDIR%\logmsg.py" info "Komutlar taranıyor..."
    python "%BASEDIR%build_command_index.py" >nul 2>"%BASEDIR%scanip_error.txt"
    if errorlevel 1 (
        call python "%ENGINEDIR%\logmsg.py" fail "Komut tarama sırasında hata oluştu. scanip_error.txt dosyasına bakınız."
    ) else (
        call python "%ENGINEDIR%\logmsg.py" system "Komutlar güncellendi."
    ...
    call python "%ENGINEDIR%\logmsg.py" system "Komutlar zaten güncel."
)
...
if "%cmdfile%"=="" (
    call python "%ENGINEDIR%\logmsg.py" fail "Geçersiz komut: '%cmd%'"
    goto :loop
)
...
if exist "%ENGINEDIR%\%cmdfile%" (
    ...
    call python "%ENGINEDIR%\logmsg.py" fail "Dosya bulunamadı: %cmdfile%"
)
